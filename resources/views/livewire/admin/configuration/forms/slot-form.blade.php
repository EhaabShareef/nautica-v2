...
@if($showModal)
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4"
         ...
         x-on:slot-form:closed.window="$nextTick(() => { document.body.style.overflow=''; })"
         wire:ignore.self>
        ...
        <div class="relative w-full max-w-xl" style="background: var(--card); border:1px solid var(--border); border-radius:var(--radius); box-shadow:0 25px 50px -12px rgba(0,0,0,0.25); animation:modal-appear .2s ease-out;">
            <div class="flex items-center justify-between px-6 py-4 border-b" style="border-color:var(--border);">
                <h3 class="text-lg font-semibold" style="color:var(--foreground);">{{ $editingSlot ? 'Edit Slot' : 'Create Slot' }}</h3>
            ...
            <form wire:submit.prevent="save" class="px-6 py-4">
                <div class="space-y-4">
                    ...
                    <div>
                        <label class="block text-sm font-medium mb-2" style="color:var(--foreground);">Zone</label>
                        <select wire:model="zone_id" class="form-input">
                            <option value="">Select zone</option>
                            @foreach($zones as $zone)
                                <option value="{{ $zone->id }}">{{ $zone->name }}</option>
                            @endforeach
                        </select>
                        @error('zone_id') <span class="text-sm mt-1 text-red-600">{{ $message }}</span> @enderror
                    </div>
                    <div>
                        <label class="block text-sm font-medium mb-2" style="color:var(--foreground);">Code</label>
                    ...
                    <div>
                        ...
                        <input type="text" wire:model="location" class="form-input" placeholder="e.g., North Dock">
                        @error('location') <span class="text-sm mt-1 text-red-600">{{ $message }}</span> @enderror
                    </div>
                    <div class="flex items-center">
                        <input type="checkbox" wire:model="is_active" class="rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50" id="slot_is_active">
                        <label for="slot_is_active" class="ml-2 text-sm" style="color:var(--foreground);">Slot is active</label>
                    </div>
                </div>
                <div class="flex items-center justify-end gap-3 mt-6 pt-4 border-t" style="border-color:var(--border);">
                    ...
                    <button type="submit" class="btn px-3 py-2 h-10 text-sm">{{ $editingSlot ? 'Update Slot' : 'Create Slot' }}</button>
                </div>
            </form>
        </div>
    </div>
